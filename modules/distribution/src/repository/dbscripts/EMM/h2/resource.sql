...
CREATE TABLE features(ID INT PRIMARY KEY auto_increment, CODE VARCHAR(8), NAME VARCHAR(32), DESCRIPTION VARCHAR(128), STATUS VARCHAR(128), MONITOR INT);

DROP TABLE IF EXISTS platform_features;
CREATE TABLE platform_features(ID INT PRIMARY KEY, PLATFORM_ID INT, FEATURE_ID INT, MIN_OS VARCHAR(8), MAX_OS VARCHAR(8), CLOUD VARCHAR(8), UI_TEMPLATE VARCHAR(32));

DROP TABLE IF EXISTS platforms;
CREATE TABLE platforms(ID INT PRIMARY KEY, OS VARCHAR(16), TYPE VARCHAR(16), DESCRIPTION VARCHAR(128));

DROP TABLE IF EXISTS devices;
CREATE TABLE devices(ID INT PRIMARY KEY, TENANT_ID INT, USER_ID VARCHAR(128), PLATFORM_ID INT, UDID VARCHAR(4096), OS_VERSION VARCHAR(64), BYOD INT, MAC_ADDRESS VARCHAR(256), STATUS VARCHAR(1), CREATED_DATE DATETIME, MODIFIED_DATE DATETIME);

DROP TABLE IF EXISTS devices_detail;
CREATE TABLE devices_detail(ID INT, SEQ_ID INT, TOKEN VARCHAR(2048));

DROP TABLE IF EXISTS policies;
CREATE TABLE policies(ID INT PRIMARY KEY, TENANT_ID INT, NAME VARCHAR(32), TYPE VARCHAR(1), STATUS VARCHAR(1), CREATED_DATE DATETIME, MODIFIED_DATE DATETIME);

DROP TABLE IF EXISTS policies_detail;
CREATE TABLE policies_detail(ID INT, SEQ_ID INT, PAYLOAD VARCHAR(2048));
...
CREATE TABLE device_policy(ID INT PRIMARY KEY, TENANT_ID INT, DEVICE_ID INT, POLICY_ID INT, POLICY_CATEGORY_ID INT, STATUS VARCHAR(1), DATETIME DATETIME);

DROP TABLE IF EXISTS device_policy_detail;
CREATE TABLE device_policy_detail(ID INT, SEQ_ID INT, PAYLOAD_UDID VARCHAR(2048));
...
CREATE TABLE device_info(ID INT PRIMARY KEY, DEVICE_ID INT, TENANT_ID INT, USER_ID VARCHAR(128), EOM INT, FEATURE_CODE VARCHAR(8), STATUS VARCHAR(1), SENT_DATE DATETIME, RECEIVED_DATE DATETIME);

DROP TABLE IF EXISTS device_info_detail;
CREATE TABLE device_info_detail(ID INT, SEQ_ID INT, SENT_MESSAGE VARCHAR(2048), RECEIVED_MESSAGE VARCHAR(2048));

DROP TABLE IF EXISTS notification;
CREATE TABLE notification(ID INT PRIMARY KEY, TENANT_ID INT, DEVICE_ID INT, NOTIFY_COUNT INT, STATUS VARCHAR(1), SENT_DATE DATETIME, RECEIVED_DATE DATETIME);
...
INSERT INTO platform_features VALUES (1, 1, 1, '4.0.3', '', 'GCM', ''), (2, 2, 1, '4.0.3', '', 'GCM', ''), (3, 3, 1, '5.0', '', 'MAPNS', ''), (4, 4, 1, '5.0', '', 'MAPNS', ''), (5, 5, 1, '5.0', '', 'MAPNS', ''), (6, 1, 2, '4.0.3', '', 'GCM', ''), (7, 2, 2, '4.0.3', '', 'GCM', ''), (8, 3, 2, '5.0', '6.1.6', 'NAPNS', ''), (9, 3, 2, '7.0', '', 'NAPNS', ''), (10, 4, 2, '5.0', '6.1.6', 'NAPNS', ''), (11, 4, 2, '7.0', '', 'NAPNS', ''), (12, 5, 2, '5.0', '6.1.6', 'NAPNS', ''), (13, 5, 2, '7.0', '', 'NAPNS', ''), (14, 1, 3, '4.0.3', '', 'GCM', ''), (15, 2, 3, '4.0.3', '', 'GCM', ''), (16, 3, 3, '5.0', '', 'MAPNS', ''), (17, 4, 3, '5.0', '', 'MAPNS', ''), (18, 5, 3, '5.0', '', 'MAPNS', ''), 
