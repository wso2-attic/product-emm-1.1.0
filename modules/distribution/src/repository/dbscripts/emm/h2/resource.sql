...
CREATE TABLE platform_operations(ID INT PRIMARY KEY auto_increment, PLATFORM_ID INT, OPERATION_ID INT, MIN_OS VARCHAR(8), MAX_OS VARCHAR(8), NOTIFIER VARCHAR(8), UI_TEMPLATE VARCHAR(32));
...
CREATE TABLE devices(ID INT PRIMARY KEY auto_increment, TENANT_ID INT, USER_ID VARCHAR(128), PLATFORM_ID INT, UDID VARCHAR(4096), OS_VERSION VARCHAR(64), OWNERSHIP VARCHAR(8), challenge_token VARCHAR(48), TOKEN TEXT, MAC_ADDRESS VARCHAR(256), STATUS VARCHAR(1), CREATED_DATE DATETIME, MODIFIED_DATE DATETIME, extra_info TEXT);
...
CREATE TABLE device_info(ID INT PRIMARY KEY auto_increment, DEVICE_ID INT, TENANT_ID INT, USER_ID VARCHAR(128), EOM INT, OPERATION_CODE VARCHAR(8), NOTIFIER VARCHAR(8), SENT_MESSAGE TEXT, RECEIVED_MESSAGE TEXT, STATUS VARCHAR(1), SENT_DATE DATETIME, RECEIVED_DATE DATETIME);

DROP TABLE IF EXISTS device_app;
CREATE TABLE device_app(DEVICE_INFO_ID INT, DEVICE_ID INT, TENANT_ID INT, PACKAGE_NAME TEXT, STATUS VARCHAR(1));

DROP TABLE IF EXISTS notification;
CREATE TABLE notification(ID INT PRIMARY KEY auto_increment, TENANT_ID INT, DEVICE_ID INT, NOTIFY_COUNT INT, NOTIFIER VARCHAR(8), STATUS VARCHAR(1), SENT_DATE DATETIME, RECEIVED_DATE DATETIME);
...
INSERT INTO platforms VALUES (1, 'ANDROID', 'PHONE', 'Android phone'), (2, 'ANDROID', 'TABLET', 'Android Tab'), (3, 'IOS', 'PHONE', 'iPhone'), (4, 'IOS', 'TABLET', 'iPad'), (5, 'IOS', 'IPOD', 'iPod');
